...
foreach $i2 (1 .. $N2) {  
  ...
  foreach $i1 (1 .. $N1) {  
    ...
    system "$bin$collect_PDB $mol1_file | grep -v '^MOLECULE ' | grep -v '^END' >> $pdbs$model ";
    system "echo 'END' >> $pdbs$model ";
    system "echo 'MOLECULE $mol2' >> $pdbs$model ";
    system "$bin$collect_PDB $mol2_file | grep -v '^MOLECULE ' | grep -v '^END' >> $pdbs$model ";
    system "echo 'END' >> $pdbs$model ";
    sleep 1;
    system "$bin$lgaexec $model $parameters > $dirres$model$res";
    sleep 1;
...
#      system "rm -f $dirres$model$res ";
    ...
    system "rm -f $pdbs$model $tmp$model$lga ";
  }
}
